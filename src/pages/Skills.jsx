          ...
          <div className="flex flex-col md:flex-row justify-between items-center mb-12 gap-6">
              ...
              {categories.map((cat) => (
                <button
                  ...
                  className={`px-6 py-2 rounded-full font-medium transition-all duration-300 capitalize text-sm ${
                    activeCategory === cat 
                      ? 'bg-accentBlue text-white shadow-[0_0_15px_rgba(0,212,255,0.6)]' 
                      : 'glass-card bg-bgSecondary text-textPrimary hover:bg-accentBlue/20 hover:text-accentBlue border-transparent'
                  }`}
                >
                  {cat}
                </button>
            ...
            <div className="flex bg-bgCard backdrop-blur-sm border border-borderColor rounded-full p-1 shadow-inner relative z-10">
              <button 
                ...
                className={`px-4 py-2 rounded-full text-sm font-bold transition-colors ${viewMode === 'grid' ? 'bg-accentViolet text-white shadow-[0_0_10px_rgba(155,93,229,0.5)]' : 'text-textMuted hover:text-textPrimary'}`}
              >
                Grid View
              </button>
              <button 
                onClick={() => setViewMode('bars')}
                className={`px-4 py-2 rounded-full text-sm font-bold transition-colors ${viewMode === 'bars' ? 'bg-accentViolet text-white shadow-[0_0_10px_rgba(155,93,229,0.5)]' : 'text-textMuted hover:text-textPrimary'}`}
              >
                List View
              </button>
            </div>
                   ...
                   <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-6">
                     {skills[activeCategory].map((skill, index) => (
                       <motion.div
                         ...
                         className="glass-card aspect-square flex flex-col items-center justify-center p-4 hover:border-accentBlue hover:-translate-y-2 hover:shadow-[0_0_20px_rgba(0,212,255,0.2)] group"
                       >
                         ...
                         <div key={index} className="w-full">
                           ...
                           <div className="w-full h-3 bg-bgSecondary rounded-full overflow-hidden border border-borderColor">
                             <motion.div 
                               ...
                               className="h-full bg-gradient-to-r from-accentBlue to-accentViolet rounded-full relative"
                             >
                               {/* Animated shine line */}
                                <motion.div 
